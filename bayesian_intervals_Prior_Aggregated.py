...
def run_bayesian_intervals(country, out_dir=OUT_DIR, save_plots=True):
    ...
    # --- Store intervals for aggregation ---
    results_for_agg = {
        "actual": actual,
        "lower_ci1": lower_ci1, "upper_ci1": upper_ci1,
        "lower_ci2": lower_ci2, "upper_ci2": upper_ci2
    }
...
# === MAIN LOOP + AGGREGATE COVERAGE ===
if __name__ == "__main__":
    countries = get_countries(OUT_DIR)
    print(f"Found {len(countries)} countries: {countries}")

    results = []
    all_actual, all_lower1, all_upper1, all_lower2, all_upper2 = [], [], [], [], []
    for c in countries:
        ...
        if res:
            results.append({
                "country": res["country"],
                "coverage_prior_mean": res["coverage_prior_mean"],
                "coverage_prior_regression": res["coverage_prior_regression"]
            })
            all_actual.append(res["actual"])
            all_lower1.append(res["lower_ci1"])
            all_upper1.append(res["upper_ci1"])
            all_lower2.append(res["lower_ci2"])
            all_upper2.append(res["upper_ci2"])

    # Aggregate coverage across all test points for both approaches
    all_actual = np.concatenate(all_actual)
    all_lower1 = np.concatenate(all_lower1)
    all_upper1 = np.concatenate(all_upper1)
    all_lower2 = np.concatenate(all_lower2)
    all_upper2 = np.concatenate(all_upper2)

    coverage1_agg = np.mean((all_actual >= all_lower1) & (all_actual <= all_upper1)) * 100
    coverage2_agg = np.mean((all_actual >= all_lower2) & (all_actual <= all_upper2)) * 100

    print("\n=== Summary of Coverages per country ===")
    if results:
        df = pd.DataFrame(results)
        print(df)
        print(f"\n[AGGREGATE] Overall Bayesian 95% CI Coverage - Prior on Mean:        {coverage1_agg:.2f}%")
        print(f"[AGGREGATE] Overall Bayesian 95% CI Coverage - Prior on Regression:  {coverage2_agg:.2f}%")
        # Only save if results exist
        df.to_csv("bayesian_interval_coverages.csv", index=False)
    else:
        print("No countries processed. Nothing to save!")
